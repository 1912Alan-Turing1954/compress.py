    ...
    def compress():
        ...
        for relPath, dirs, files in os.walk(root):
            if name in dirs:
                ...
                print(f'Compress percentage: {round(100.0 - (float(before)/float(sys.getsizeof(after)) * 100.0), 2)}%')
                print('---------------')
    ...
    def decompress():
        ...
        for relPath, dirs, files in os.walk(root):
            if name in dirs:
                ...
                print(f'Decompress percentage: {round(100.0 - (float(before)/float(sys.getsizeof(after)) * 100.0), 2)}%')
                print('---------------')
